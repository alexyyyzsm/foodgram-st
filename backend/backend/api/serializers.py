...
class СustomizeUserSerializer(UserSerializer):
    ...
    def get_is_subscribed(self, data):
        #return self.checking_fields(model=Subscription, data=data)
    ...
    # def checking_fields(self, model, data):
    #     req = self.context.get('request')
    #     if not req or req.user.is_anonymous:
    #         return False
    #     if model == Subscription:
    #         return Subscription.objects.filter(
    #             user=req.user, subscribed_to=data
    #         ).exists()
    #     return model.objects.filter(user=req.user, recipe=data).exists()


class AvatarUserSerializer(СustomizeUserSerializer):
    ...
    def validate(self, data):
        if not data.get('avatar'):
            raise serializers.ValidationError('Аватарку не удалось добавить!!!')
        return data
