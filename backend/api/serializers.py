...
class UserDetailSerializer(UserSerializer):
    ...
    class Meta(UserSerializer.Meta):
        ...
        """
        fields = list(UserSerializer.Meta.fields) + ['is_subscribed', 'avatar', 'email']

    def get_is_subscribed(self, obj):
        request = self.context.get('request')
        return (request and request.user.is_authenticated and
                obj != request.user and obj.subscribed_to.filter(user=request.user).exists())


class CustomUserCreateSerializer(UserDetailSerializer):
    ...
    class Meta(UserDetailSerializer.Meta):
        """
        Наследуется от Meta UserDetailSerializer.
        Добавлено поле 'password' для создания пользователя.
        """

        fields = [field for field in list(UserDetailSerializer.Meta.fields)
                  if field != 'avatar'] + ['password', 'email']
        extra_kwargs = {
        ...
        for field in fields:
            if field not in data:
                raise serializers.ValidationError(
                    {f'{field}': f'Поле "{field}" обязательно к заполнению.'}
                )

        if data['email'] == data['username']:
            raise serializers.ValidationError(
                'Имя пользователя не должно совпадать с адресом электронной почты!'
            )
...
class IngredientInRecipeCreateViewSerializer(serializers.ModelSerializer):
    ...
    def validate_amount(self, value):
        if value < MIN_AMOUNT_INGREDIENTS:
            raise serializers.ValidationError(
                f'Количество ингредиента должно быть больше {MIN_AMOUNT_INGREDIENTS}!'
            )
        return value
